...
public class WorkshopBuffView : Module
{
    ...
    public void OpenHelpPanel(string type) {
        string help = type switch {
            "id" => "印记的序号只能输入-100001以下的数字(特性纹章除外)\n" + 
            "特性和纹章类印记的序号需要填写900000(纹章800000)+该精灵基础型态之序号（去除负号）\n" +
            "例如：波克尔的序号为 -12，基础型态(皮皮)的序号为 -10，\n" + 
            "因此皮皮的特性印记需要填写900010，纹章印记需要填写800010\n\n" +
            "天气类的序号需要填写510001到599999之中的数字",
            
            "copy" => "打上同序号印记时的处理方式\n" + 
                "特性、纹章类的覆盖须为【覆盖旧印记】",

            "turn" => "印记预设回合数，输入负数表示印记永久存在\n" + 
                "技能或其他效果打上印记时若未指定回合数，则会默认为预设回合数",

            "icon" => "点击左方格子上传图片，按下取消可以清除\n" +
                "请上传长宽比为1:1的png档案，不接受其他格式\n\n" + 
                "若想使用其他印记图案，可以在【其他】填写「res=现有序号」例如：res=10\n" +
                "特性和纹章类印记无须上传图片，但序号需要填写900000(纹章800000)+该精灵基础型态之序号（去除负号）\n" +
                "例如：波克尔的序号为 -12，基础型态(皮皮)的序号为 -10，\n" + 
                "因此皮皮的特性印记需要填写900010，纹章印记需要填写800010\n" +
